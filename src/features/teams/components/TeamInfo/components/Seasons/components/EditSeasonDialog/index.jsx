...
const initialState = {
  ...
  allowMeCoach: false,
  allowEditStartDate: true,
  allowEditEndDate: true
};

class EditSeasonDialog extends Component {
  state = initialState;
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (isOpen !== this.props.isOpen) {
      if (isOpen) {
        ...
        const nonCompetitiveEvents = initialInfo.nonCompetitiveEvents.map(
          info => {
            const startTime = moment(info.times.start, "hh:mm A");
            const endTime = moment(info.times.end, "hh:mm A");
            return {
        ...
        return [
          ...
          <Button
            ...
            handleClick={() => {
              ...
              isValid && editSeason(this.getSeasonInfo());
            }}
          >
            Update season
          </Button>
  ...
  render() {
    ...
    return (
      <Dialog
        isOpen={isOpen}
        size="medium"
        heading={`Edit Season for ${teamName}`}
        actions={actions}
        hasSteps
        numberOfSteps={5}
        currentStep={step}
...
export default injectSheet(styles)(EditSeasonDialog);
